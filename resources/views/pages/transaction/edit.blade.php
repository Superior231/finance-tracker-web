<x-app-layout :title="$title">
    <div x-data="editTransaction({
        typeInitial: '{{ old('type', $transaction->type) }}',
        categories: @js($categories->map(fn($c) => ['id' => $c->id, 'name' => $c->name, 'type' => $c->type])),
        oldCategoryId: '{{ old('category_id', $transaction->category_id) }}',
        existingReceiptUrl: {{ $transaction->receipt ? Js::from(asset('storage/receipts/' . $transaction->receipt)) : 'null' }},
        existingItems: @js($transaction->items->map(fn($it) => [
            'item_name' => $it->item_name,
            'qty' => $it->qty,
            'price' => $it->price,
        ])),
        existingOcrData: {{ Js::from($transaction->ocr_data) }},
        amountInitial: '{{ old('amount', $transaction->amount) }}'
    })">
        <x-slot name="header">
                ...
                <div class="p-3 card-body p-lg-4">
                    ...
                    <div class="mt-6">
                        <form method="POST" action="{{ route('transactions.update', $transaction) }}"
                            enctype="multipart/form-data" class="space-y-6" id="form"
                            @receipt-selected="autoParseReceipt($event.detail.file)"
                            @receipt-cleared="clearOcrData()">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="type" :value="type">
                            <input type="hidden" name="ocr_data" :value="ocrDataRaw">
                            <input type="hidden" name="remove_receipt" :value="removeReceipt ? '1' : '0'">

                            <div class="sm:col-span-2">
                                <label class="block text-sm font-medium text-gray-700">Receipt Image</label>

                                <div x-data="receiptUploader({ existingUrl: existingReceiptUrl })" class="mt-2">
                                    ...
                                    <!-- Preview (existing atau baru dipilih) -->
                                    <div x-show="previewUrl" x-cloak
                                        ...
                                        <!-- badge untuk gambar lama -->
                                        <div x-show="isExisting" x-cloak
                                            class="absolute px-2 py-1 text-xs font-medium text-green-100 rounded-lg shadow bg-green-600/70 top-3 left-3">
                                            Current receipt
                                        </div>
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                    ...
                                    <x-select-input id="category_id" name="category_id" class="block w-full mt-1"
                                        ...
                                        <template x-for="c in filteredCats" :key="c.id">
                                            <option :value="c.id" x-text="c.name"
                                                :selected="String(c.id) === String(category_id)"></option>
                                        </template>
                                    </x-select-input>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="date" type="date" name="date" class="block w-full mt-1"
                                        :value="old('date', \Illuminate\Support\Carbon::parse($transaction->date)->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <x-textarea id="note" type="text" name="note"
                                    class="block w-full mt-1">{{ old('note', $transaction->note) }}</x-textarea>
                            ...
                            <div class="flex items-center justify-center">
                                <x-primary-button class="flex items-center justify-center w-full text-center"
                                    ...
                                    <span class="text-center" id="textBtn">Update</span>
                                </x-primary-button>
                            </div>
                        </form>
                    </div>
        ...
        @push('scripts')
            <script>
                ...
                function editTransaction(init) {
                    return {
                        type: init.typeInitial || 'expense',
                        categories: init.categories || [],
                        category_id: init.oldCategoryId || '',
                        existingReceiptUrl: init.existingReceiptUrl || null,
                        items: (init.existingItems || []).map(it => ({
                            item_name: it.item_name ?? '',
                            qty: it.qty ?? 1,
                            price: it.price ?? 0,
                            priceFormatted: it.price ? this_formatRupiah(it.price) : ''
                        })),
                        previewUrl: null,
                        // FIX: ocrDataRaw sekarang diisi dari data existing,
                        // supaya kalau user tidak mengganti gambar, data OCR lama tidak hilang
                        ocrDataRaw: init.existingOcrData ? JSON.stringify(init.existingOcrData) : '',
                        removeReceipt: false,
                        ...
                        init() {
                            // set amount awal dari data transaksi / old()
                            this.amountFormatted = init.amountInitial ? this.formatRupiah(init.amountInitial) : '';
                        ...
                        // dipanggil saat gambar struk dihapus (baik yang lama maupun baru)
                        clearOcrData() {
                            this.ocrDataRaw = '';
                        ...
                        async autoParseReceipt(file) {
                            ...
                            setStatus('Processing receipt image...', true);
                            ...
                            try {
                                ...
                                // gambar baru menggantikan item lama dengan hasil parsing baru
                ...
                // helper karena formatRupiah dipakai sebelum instance x-data terbentuk penuh
                function this_formatRupiah(num) {
                    if (num === null || num === undefined || num === '') return '';
                    const numeric = String(num).replace(/[^0-9]/g, '');
                    return numeric ? 'Rp ' + new Intl.NumberFormat('id-ID').format(numeric) : '';
                }

                function receiptUploader(init) {
                    return {
                        previewUrl: init.existingUrl || null,
                        isExisting: !!init.existingUrl,
                        fileName: init.existingUrl ? init.existingUrl.split('/').pop() : '',
                        fileSize: 0,
                        drag: false,
                        error: '',
