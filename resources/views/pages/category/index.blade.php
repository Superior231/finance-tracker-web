        ...
        <div class="mb-5 shadow-sm card">
            <div class="p-3 card-body p-lg-4">
                <div class="actions d-flex justify-content-between align-items-center">
                    ...
                    <button class="gap-1 px-4 py-2 text-light btn btn-primary d-flex align-items-center rounded-10"
                        data-bs-toggle="modal" data-bs-target="#tambah-kategori-modal">
                                ...
                                <tr class="align-middle">
                                    ...
                                    <td class="!text-xs md:text-base">
                                        <div class="type-info d-flex align-items-center justify-content-center pe-3">
                                            @if ($item->type == 'income')
                                                <span
                                                    class="px-2 py-1 text-green-700 capitalize bg-green-100 border !border-green-300 rounded-pill !text-xs">{{ $item->type }}</span>
                                            @else
                                                <span
                                                    class="px-2 py-1 text-red-700 capitalize bg-red-100 border !border-red-300 rounded-pill !text-xs">{{ $item->type }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div
                                            class="gap-2 actions d-flex align-items-center justify-content-center pe-3">
                                            <button
                                                class="p-2 rounded-05 btn btn-primary d-flex align-items-center justify-content-center"
                                                onclick="editCategory('{{ $item->id }}', '{{ Auth::user()->id }}', '{{ $item->type }}', '{{ $item->name }}')"
                                                data-bs-toggle="modal" data-bs-target="#edit-kategori-modal">
                                                <i class='p-0 m-0 bx bxs-pencil'></i>
                                            </button>
                                            <form id="delete-category-form-{{ $item->id }}"
                                                action="{{ route('categories.destroy', $item->id) }}" method="POST">
                                                @csrf @method('DELETE')

                                                <button type="button"
                                                    class="p-2 rounded-05 btn btn-danger d-flex align-items-center justify-content-center"
                                                    onclick="confirmDeleteCategory({{ $item->id }})">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="mb-2">
                            ...
                            <x-select-input name="type" id="type" class="block w-full mt-1" x-model="type"
                                required>
                        ...
                        <x-text-input id="name" type="text" name="name" class="block w-full mt-1"
                            placeholder="Enter a new category name" required />
                    </div>
                    <div class="border-0 modal-footer">
                        <button type="submit" class="px-4 btn btn-primary rounded-10"
                            id="tambah-kategori-btn">Create</button>
                    </div>
                </div>
            </form>
        </div>
                ...
                <form id="edit-category-form" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-2">
                            ...
                            <x-select-input id="edit-type" name="type" class="block w-full mt-1"
                                x-model="edit-type" required>
                                <option value="income">Income</option>
                                <option value="expense">Expense</option>
                            </x-select-input>
                        </div>
                        ...
                        <x-text-input id="edit-name" type="text" name="name" class="block w-full mt-1"
                            placeholder="Enter category name" required />
                    </div>
                    <div class="border-0 modal-footer">
                        <button type="submit" id="edit-kategori-btn"
                            class="px-4 btn btn-primary rounded-10">Save</button>
                    </div>
                </form>
            </div>
        </div>
